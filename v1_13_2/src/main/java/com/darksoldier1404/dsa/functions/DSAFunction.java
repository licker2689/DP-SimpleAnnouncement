        ...
        plugin.announcementTask = plugin.getServer().getScheduler().runTaskTimer(plugin, new Runnable() {
            ...
            public void run() {
                ...
                if (plugin.config.getBoolean("Settings.random")) {
                    ...
                    plugin.getServer().broadcastMessage(plugin.prefix + ChatColor.translateAlternateColorCodes('&', announcements.get(index)));
                    announcements.remove(index);
                } else {
                    plugin.getServer().broadcastMessage(plugin.prefix + ChatColor.translateAlternateColorCodes('&', announcements.get(0)));
